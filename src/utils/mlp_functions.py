...
from utils.loss_functions import binary_cross_entropy, categorical_cross_entropy, sparse_categorical_cross_entropy
...
LOSS_FUNCTIONS: dict[str, callable] = {
    "sparseCategoricalCrossentropy": sparse_categorical_cross_entropy,
...
def create_model_config(
    ...
    # Validate and set solver
    if solver not in ["sgd", "momentum"]:
        print(f"{Fore.YELLOW}Error: Unknown solver '{solver}'."
              f'Available choices: ["sgd", "momentum"]{Style.RESET_ALL}')
        exit(1)
    
    solver_config = {
        'name': solver,
        'learning_rate': learning_rate,
        'momentum': 0.9 if solver == "momentum" else None
    }
    ...
    if activation not in ACTIVATIONS_FUNCTIONS:
        print(f"{Fore.YELLOW}Error: Unknown activation function '{activation}'.\n"
              f"Available choices: {list(ACTIVATIONS_FUNCTIONS.keys())}{Style.RESET_ALL}")
        exit(1)
    ...
    if loss not in LOSS_FUNCTIONS:
        print(f"{Fore.YELLOW}Error: Unknown loss function '{loss}'.\n"
              f"Available choices: {list(LOSS_FUNCTIONS.keys())}{Style.RESET_ALL}")
        exit(1)
...
def init_network(layer_sizes, weight_initializer, random_seed=None):
    ...
    for i in range(len(layer_sizes) - 1):
        ...
        match weight_initializer:
            ...
            case _:
                print(f"{Fore.YELLOW}Error while initializing weights{Style.RESET_ALL}")
                exit(1)
...
def fit_network(X_train, y_train, X_val, y_val, config, early_stopping_config=None):
    ...
    W, b = init_network(layer_sizes, config['weight_initializer'], config['random_seed'])
    ...
    for epoch in range(config['epochs']):
        ...
        for i in range(0, len(X_train), config['batch_size']):
            ...
            # Actualizar usando la función del optimizador correspondiente
            if config['solver']['name'] == 'sgd':
                W, b, optimizer_state = sgd(W, b, dW, db, config['solver']['learning_rate'], optimizer_state)
            else:  # momentum
                W, b, optimizer_state = momentum(W, b, dW, db, config['solver']['learning_rate'], 
                                              config['solver']['momentum'], optimizer_state)
        ...
        print(f"{Fore.MAGENTA}epoch {Fore.CYAN}{epoch+1}/{config['epochs']}"
              ...
              f"{Fore.GREEN} - {Fore.MAGENTA}val_acc: {Fore.CYAN}{val_accuracy:.4f}{Style.RESET_ALL}")
        ...
        if early_stopping_state is not None:
            ...
            if should_stop:
                print(f"{Fore.GREEN}Early stopping triggered at epoch {epoch + 1}{Style.RESET_ALL}")
